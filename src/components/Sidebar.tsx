...
import { BotIcon, EyeIcon, Loader2Icon, UserIcon } from 'lucide-react';
...
function Sidebar({isMenuOpen,project , setProjects,isGenerating,setIsGenerating}:SidebarProps) {
  return (
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 bprder-gray-800 transition-all ${isMenuOpen ? "max-sm:w-0 overflow-hidden" : "wfull"}`}>
            ...
            if(isMessage){
                ...
                return (
                    <div key={msg.id} className={`flex items-center gap-3 ${isUser ? "justify-end":"justify-start"}`}>
                        {!isUser && (
                            <div className='w-8 h-8 rounded-full bg-linear-to-br from-indigo-600 to-indigo-700 flex items-center jsutify-center'>
                                <BotIcon className='size-5 text-whhite'/>
                            </div>
                        )}
                        <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? " bg-linear-to-r from-indigo-500  to-indigo-600 text-white ronded-tr-none": " rounded-tl-none bg-gray-800 text-gray-100"}`}>
                            {msg.content}
                        </div>
                        {isUser && (
